...
- 구조체 Struct (3)
  - 공용체 (union)
    - 사실 **공용체(union)** 는 많이 사용하는 기능은 아니지만, C언어에서 제공하는 것 중 하나이니 간단하게 넘어간다.
    - 공용체는 구조체와는 달리 메모리를 '공유'한다. ![union.webp](./src/img/union.webp)
    - 위 그림을 보면 알 수 있듯, 공용체의 각 멤버들의 메모리 시작 주소는 모두 동일하다. 따라서 위 그림의 union A의 경우 j의 값을 변경함으로써 i의 값을 변경할 수 있고, i의 값을 변경함으로써 j의 값을 변경할 수 있게 된다.
...
- a의 j멤버 값을 설정하지 않았음에도, i에 0x12345678을 대입하자 j의 값이 78로 잘 나왔다. 그런데 78이 아니라 0x12가 나와야 하는게 아니냐고 생각할 수 있다. 분명히 i와 j에 동일한 주소값에 위치해 있고 i가 0x12345678로 메모리 상에 있다면 j는 처음 두 개인 0x12가 되어야 하는게 아닌가? 물론 이 생각은 옳지만, 컴퓨터에서는 수를 이런식으로 보관하지 않는다.
...
- 구조체 Struct (3)
  - 빅 엔디안 (Big Endian), 리틀 엔디안 (Little Endian)
    - 컴퓨터에서 메모리에 수를 저장할 때
      ...
      - 현재 대부분은 리틀 엔디안 방식을 사용하고 있고, 일부 컴퓨터만 빅 엔디안 방식을 사용하고 있다.\

- 빅 엔디안에서 수를 저장하는 방법
  - 아래 그림과 같이 상식적으로 수를 저장하게 된다.
![bigEndian.webp](./src/img/bigEndian.webp)
- 리틀 엔디안에서 수를 저장하는 방법
  - 아래 그림과 깉이 1바이트씩 역으로 보관함을 알 수 있다. 따라서 출력했던 j값은 0x78이 된다.
  - j를 short형으로 지정했으면 0x7856가 나올지, 0x5678이 나올지에 대해 아래 코드를 보자.
![littleEndian.webp](./src/img/littleEndian.webp)
...
- 코드 분석
  - 실행 시켰을 떄 결과로는 0x5678로 나온다.
  - 메모리 상을 살펴보면 분명히 j에 해당하는 부분은 0x7856으로 출력되어야 하지만 컴퓨터는 정상적으로 0x5678을 출력하였다.
![litteEndianShort.webp](./src/img/littleEndianShort.webp) 
- 이번에는 j가 short형(2바이트)이므로 위와 같이 2바이트를 차지하게 된다. 이 때 i의 값을 출력하면 얼마가 나올까? 당연히 0x12345678이 될것이다. 왜냐하면 컴퓨터는 자신이 메모리에 수를 '리틀 엔티안 방식'을 저장하고 있다는 사실을 알고 있기 때문이다. 따라서 이를 출력할 때에는 적절한 변환을 통해 0x12345678을 출력할 것이다.
- j의 경우도 마찬가지이다. j는 현재 '78 56'부분을 가리키고 있지만 컴퓨터는 j가 리틀 엔디안 형식으로 이루어 졌다는 것을 알기에 j를 출력할 때에는 적절히 변환하여 0x5678을 출력하게 될 것이다.

> 공용체에 관한 설명은 여기서 끝이난다. 아마 공용체를 접할 가능성이 100번중 1번 나올까말까 해서 큰 의미가 없다고 생각한다. 사실 공용체에 대해 배운 것보다 엔디안에 대해 배운 것이 훨씬 중요하다. 엔디안에 대해 잊는 일은 없길 바란다.
...
- 구조체 Struct (3)
  - 열거형 (Enum)
    - 프로그래밍을 하다보면 각 데이터에 수를 대응 시키는 경우가 많다.
